...
func (s *Server) registerAdminRoutes(r *gin.RouterGroup) {
	...
	superAdminGroup.POST("/users", s.handleAdminCreateUser)
	superAdminGroup.DELETE("/users/:id", s.handleAdminDeleteUser)
	superAdminGroup.PATCH("/users/:id/status", s.handleAdminUpdateStatus)
	superAdminGroup.POST("/users/:id/admin", s.handleAdminToggleAdmin)
	superAdminGroup.PATCH("/users/:id/group", s.handleAdminAssignUserGroup)
